...
import {
  ...
  RefreshCw, CalendarClock, Activity,
} from "lucide-react";
...
/* Reusable style objects - avoid creating new objects per render */
...
export default function DashboardOverviewPage() {
  ...
  const fetchSummary = useCallback(async () => {
    setLoading(true);
    try {
      const res = await analyticsApi.summary(reportRange) as { success: boolean; data: SummaryResponse; message?: string };
      if (res.success) {
        setData(res.data);
      } else {
  ...
  const lastUpdate = kpi?.last_update
    ? format(new Date(kpi.last_update), "dd MMM yyyy HH:mm")
    : "-";
    ...
    <AppLayout title="Overview">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground ml-7">Real-time monitoring across all datasets</p>
        </div>
        <Button variant="outline" size="sm" onClick={fetchSummary} disabled={loading} className="gap-1.5 h-8 shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3 mb-6">
        <KpiCard
          title="Total SOW"
          value={loading ? "..." : formatNumber(kpi?.total ?? 0)}
          icon={FolderKanban}
          iconColor="text-sky-600"
        />
        <KpiCard
          title="RFS / Completed"
          value={loading ? "..." : formatNumber(kpi?.completed ?? 0)}
          icon={CheckCircle2}
          iconColor="text-emerald-600"
        />
        <KpiCard
          title="NY / Remaining"
          value={loading ? "..." : formatNumber(kpi?.remaining ?? 0)}
          icon={Clock}
        ...
        <KpiCard
          ...
          value={loading ? "..." : formatNumber(kpi?.dropped ?? 0)}
          icon={AlertTriangle}
          iconColor="text-rose-600"
        />
        <KpiCard
          title="Issues / Blocking"
          value={loading ? "..." : formatNumber(kpi?.issues ?? 0)}
          icon={TrendingUp}
          iconColor="text-orange-600"
        />
        <KpiCard
          title="Last Update"
          value={loading ? "..." : lastUpdate}
          icon={CalendarClock}
